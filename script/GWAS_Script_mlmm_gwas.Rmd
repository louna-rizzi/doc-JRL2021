...
# pour G y[which(y> 0.10)]<-NA

hist(y, col = "grey",  nclass=20, xlab = "BLUP biomasse", main=paste("Distribution of", names(myY)[i]))

# on élimine deux données (arbitraire) qui ont des valeurs extrêmes
#y[which(y> 0.10)]<-NA
...
```
C'est une matrice carrée qui donne les relations deux à deux pour toutes les lignées. ici elle est 176 x 176.
...
res_eBIC
```
Le meilleur modèle est celui qui a le plus petit eBIC. Ici il n'accepte aucun SNP significatif mais on va continuer quand même.
...
Voilà le SNP qui passe la barre
```{r}
res_threshold
...
```
Il y a un pic sur le chromosome 13 (donc le 7A).
...
effects
```
Ici le génotype 00(aa) a une différence de -0.032 avec le génotype 11 (AA).
